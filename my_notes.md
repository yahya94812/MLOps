...
Think of images like inheritance :
...
In reality, many official images are already built this way.
For example, Python images often internally start from Debian or Alpine Linux images.
...
Docker combines them into one unified filesystem using a union filesystem mechanism (like OverlayFS on Linux).

To the container, it looks like one normal filesystem.
...
can still produce a large image because the installation layer still exists underneath.
...
```
docker run -it ubuntu bash
most common ways to start an interactive container in Docker.
...
Together:
-it
gives you a proper interactive shell session.
```
